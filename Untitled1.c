...
{
    if(b>a+1 || b<a-1 || c.row>c.rowch+1 || c.row <c.rowch-1)                                    /*check for wrong move*/
    {
        printf("invalid move \n");
        erre_ch=1;
    }
...
{
g[c.collum][c.row].vg=g[c.collumch][c.rowch].vg;
g[c.collumch][c.rowch].vg='\0';
g[c.collum][c.row].player=g[c.collumch][c.rowch].player;
g[c.collumch][c.rowch].player=0;
}
